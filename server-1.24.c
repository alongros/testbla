...
int main()
{
    ...
    while(!finished)
    {
        ...
        if(get>7) {
          ...
          // security fix for heap overflow in version 1.23: 
          // output fixed string 'XXXX' if length does not match what we are expecting
          if(request->type==0) {
            ...
            if (request->length == 4) {
              ...
              memcpy(&response->payload,(void*)"XXXX",4);
            }
          ...
          // version command:
          else if(request->type==0x11111111 && memcmp(&request->payload,(void*)"version",7)==0){
            struct msg *response = (struct msg*) malloc(8+12);
            response->length=12;
            response->type=0x11111111;
            memcpy(&response->payload,(void*)"version:1.24",12);
            write(newSocket,(void *)response,8+response->length);
            free(response);
          }
          else {
